...
function buildDqNotebook(config) {
  ...
  const functionsLines = [
    ...
    `\n`,
    `def _safe_json(df):\n`,
    `    return F.to_json(F.struct(*[F.col(f"\`{c}\`").cast("string").alias(c) for c in df.columns]))\n`,
    `\n`,
    ...
    `        _safe_json(df).alias("raw_values")).limit(100) if metric < threshold else None\n`,
    ...
    `                _safe_json(df).alias("raw_values")).limit(100)\n`,
    `        else:\n`,
    `            failed = df.filter(F.col(col).isNull()).select(_safe_json(df).alias("raw_values")).limit(100)\n`,
    `    else:\n`,
    `        failed = None\n`,
    `    return metric, metric >= threshold, failed, total\n`,
    `\n`,
    ...
    `        _safe_json(df).alias("raw_values")).limit(100) if not passed else None\n`,
  ...
  const exportJsonLines = [
    ...
    `    "results": all_results,\n`,
    `}\n`,
    `mssparkutils.fs.put(f"{run_path}/summary.json", json.dumps(summary), True)\n`,
    `\n`,
    `failed_export = {"run_id": run_id, "failed_rows": all_failed_rows}\n`,
    `mssparkutils.fs.put(f"{run_path}/failed_rows.json", json.dumps(failed_export), True)\n`,
    `\n`,
